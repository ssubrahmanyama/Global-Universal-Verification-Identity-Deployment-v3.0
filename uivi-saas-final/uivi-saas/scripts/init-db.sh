...
# ✅ Use connection string (best practice)
PG="psql postgresql://uivi:uivi@db:5432/uivi_master"

HASH='$2a$10$YGDWnFBmEzeBsaGnGGhZDO/XF3YSSb.MLiLl0nkRnMz8jSqp.U1wS'

echo "Waiting for Postgres..."

# ⏳ Retry with timeout (prevents infinite loop)
for i in {1..30}; do
  if $PG -c '\q' >/dev/null 2>&1; then
    echo "Ready."
    break
  fi
  echo "⏳ Waiting... ($i)"
...
create_tenant() {
  ...
  psql postgresql://uivi:uivi@db:5432/postgres -c "CREATE DATABASE $db;" 2>/dev/null || true

  # Schema inside tenant DB
  psql postgresql://uivi:uivi@db:5432/$db << 'TSQL'
CREATE TABLE IF NOT EXISTS users (
  id VARCHAR(64) PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS consent_preferences (
  ...
  mode VARCHAR(32) DEFAULT 'PASSIVE',
  updated_at TIMESTAMP DEFAULT NOW()
);
...
psql postgresql://uivi:uivi@db:5432/uivi_iit_bombay -c \
"INSERT INTO users(id,email,full_name,role,password_hash)
VALUES
('u1','issuer@iit-bombay.uivi.app','Prof. Sharma','institution','$H'),
('u2','student@iit-bombay.uivi.app','Rahul Mehta','user','$H')
ON CONFLICT DO NOTHING;"

psql postgresql://uivi:uivi@db:5432/uivi_google_hr -c \
"INSERT INTO users(id,email,full_name,role,password_hash)
VALUES
('u3','hr@google-hr.uivi.app','Priya Singh','hr','$H')
ON CONFLICT DO NOTHING;"

psql postgresql://uivi:uivi@db:5432/uivi_naac_reg -c \
"INSERT INTO users(id,email,full_name,role,password_hash)
VALUES
('u4','regulator@naac-reg.uivi.app','Dr. Kumar','regulatory','$H')
ON CONFLICT DO NOTHING;"

psql postgresql://uivi:uivi@db:5432/uivi_fraud_l1 -c \
"INSERT INTO users(id,email,full_name,role,password_hash)
VALUES
('u5','analyst@fraud-l1.uivi.app','Arun Das','fraud_monitor','$H')
ON CONFLICT DO NOTHING;"
...
# ─────────────────────────────────────────────────────────
psql postgresql://uivi:uivi@db:5432/uivi_iit_bombay -c \
"INSERT INTO certificates(id,uivid,credential_hash,holder_name,holder_email,degree_type,degree_name,roll_number,passing_year,grade,issuer_id,issuer_name)
VALUES
('cert-1','UIVI-2024-DEMO01','hash_demo_001','Rahul Mehta','student@iit-bombay.uivi.app','B.Tech','Computer Science & Engineering','IIT-2024-CS-001',2024,'First Class','u1','IIT Bombay')
ON CONFLICT DO NOTHING;"
